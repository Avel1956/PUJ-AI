...
def _tab_chat_admin(usuario):
    ...
    with col_hist:
        ...
        if st.button("➕ Nueva conversación", key=f"nueva_conv_adm_{asignatura}", use_container_width=True):
            ...
            if f"conv_flag_adm_{asignatura}" in st.session_state:
                del st.session_state[f"conv_flag_adm_{asignatura}"]
            st.rerun()

        if convs_resp.data:
            for c in convs_resp.data:
                label = f"{c['titulo'][:40]} — {c['created_at'][:10]}"
                if st.button(label, key=f"load_conv_adm_{c['id']}", use_container_width=True):
                    msgs_resp = (
                    ...
                    if f"conv_flag_adm_{asignatura}" not in st.session_state:
                        st.session_state[f"conv_flag_adm_{asignatura}"] = True
                    st.rerun()
        else:
